    ...
    try {
      ...
      if (!currentChatId) {
        const { data: newChat, error: chatError } = await supabase
          .from('chats')
          .insert({ user_id: user.id, subject, messages: [userMsg] })
          .select('id, messages')
          .single();
      ...
      if (ttsEnabled) {
        ...
        utterance.onend = () => setSpeakingId(null);
        window.speechSynthesis.speak(utterance);
        setSpeakingId(`msg-${messages.length + 1}`);
      }

      // Update user stats
      await supabase
        .from('profiles')
        .update({ topics_learned: (user?.topicsLearned || 0) + 1, last_active: new Date().toISOString() })
        .eq('id', user.id);
    } catch (err) {
      setMessages((prev) => [
        ...prev,
    ...
    <div className="chat-page">
      <div className="chat-header">
        <div className="chat-header-info">
          ...
          <div>
            ...
            <p className="chat-subtitle">Ask any STEM question — step-by-step explanations</p>
          </div>
        </div>
        <div className="chat-header-actions">
          <button
            className={`chat-voice-toggle ${ttsEnabled ? 'chat-voice-active' : ''}`}
          ...
          <select value={subject} onChange={(e) => setSubject(e.target.value)} className="chat-subject-select">
            {SUBJECTS.map((s) => (
      ...
      <div className="chat-messages">
          ...
          <div key={i} className={`chat-message chat-message-${msg.role}`}>
            ...
            <div className="chat-message-content">
              <div className="chat-message-sender">
                {msg.role === 'user' ? 'You' : 'STEMAI Tutor'}
              </div>
              <div className="chat-message-text">{msg.content}</div>
              {msg.role === 'assistant' && (
                <button
                  className={`chat-speak-btn ${speakingId === `msg-${i}` ? 'chat-speak-active' : ''}`}
                  onClick={() => speakText(msg.content, `msg-${i}`)}
                  title={speakingId === `msg-${i}` ? 'Stop speaking' : 'Read aloud'}
                >
                  {speakingId === `msg-${i}` ? <VolumeX size={14} /> : <Volume2 size={14} />}
                </button>
              )}
            </div>
          </div>
        ...
        {loading && (
          <div className="chat-message chat-message-assistant">
            ...
            <div className="chat-message-content">
              <div className="chat-message-sender">STEMAI Tutor</div>
      ...
      <form className="chat-input-area" onSubmit={handleSend}>
        ...
        <input
          ref={inputRef}
          type="text"
          placeholder={isListening ? 'Listening...' : 'Ask a STEM question...'}
          value={input}
